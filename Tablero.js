...
class Tablero {
  ...
  */
  dibujar() {
    ...
    // BUCLE FOR QUE DETERMINA COLUMNAS Y FILAS PARES E IMPARES PARA DARLE COLOR A AMBOS GRUPOS.
    for (let columna = 0; columna < this.columnas; columna++) {
      for (let fila = 0; fila < this.filas; fila++) {
        if ((columna + fila) % 2 == 0) { //Si la col y fil son divisibles por 2 (es decir par) rellenar white.
          fill("white");                            // '%2 == 0' significa que su division por 2 tenga un resto de 0
        } else {  //Si son impar, con #1b1b32
          fill("#1b1b32");
        }
